...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      if(size_t(_ptr) + toSizeT(t) <= size_t(_end))
      {
         ret = (T*)_ptr;
         _ptr = _ptr + toSizeT(t);
         return true;
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      MemRecycleList<T>* pre_tmp; 
      while(tmp != NULL)
      {
         if(tmp -> getArrSize() == n) return tmp;
         pre_tmp = tmp;
         tmp = tmp -> getNextList();
      }
      MemRecycleList<T>* new_arr = new MemRecycleList<T>(n);
      pre_tmp->setNextList(new_arr);
   ...
   T* getMem(size_t t) {
      ...
      //TODO
      MemRecycleList<T>* recyList = getMemRecycleList(n);
               // cout << "plz2\n";

      if(recyList->_first != NULL)
      {
               // cout << "plz\n";
         ret = recyList->popFront();
         #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
         #endif // MEM_DEBUG
      }
      ...
      {
         bool enough = _activeBlock->getMem(t,ret);
         size_t remainSize = _activeBlock -> getRemainSize();
         if(!enough)
         {
            if(remainSize >= toSizeT(S))
            {
               // cout << "plz\n";
               size_t rn = getArraySize(downtoSizeT(remainSize));
               #ifdef MEM_DEBUG
               cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
               #endif // MEM_DEBUG
               size_t tot_mem = rn*S+SIZE_T;
               _activeBlock->getMem(tot_mem,ret);
               // cout << "plz\n";
