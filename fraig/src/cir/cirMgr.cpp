...
CirMgr::printSummary() const
{
  ...
  //   cout << dfsOrder[i] << " ";
  // cout << endl;
  // for(size_t i = 0; i < AIGinDFSOrder.size(); i++)
  //   cout << AIGinDFSOrder[i] << " ";
  // cout << endl;
  // for(size_t i = 0; i < notInDFS.size(); i++)
  //   cout << notInDFS[i] << " ";
  // cout << endl;
   /*for(unsigned int i=0;i<=M+O;i++)
   {
      if(gates[i])
      {
        ...
        if(gates[i]->gateType == PO_GATE)
        {
          // tmp->gateType = UNDEF_GATE ;
                  // CirIOGate* tmp = reinterpret_cast<CirIOGate*>(gates[i]);

          cout << tmp->name << " " cout << tmp->id <<endl;
          // cout << "yes\n";
        }
      }
   }*/
...
CirMgr::writeAag(ostream& outfile) const
{
   ...
   outfile << "c\n" << "OMG!!! The code is so long!!! Jui-Che (Chris) Wu"<<endl;
}
