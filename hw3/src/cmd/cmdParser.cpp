...
CmdParser::parseCmd(string& option)
{
   ...
   {
      string n1;
      size_t END = myStrGetTok(str, n1);
      if(str == n1)
      {  
         option = "";
      }
      else option = str.substr(END+1, str.size()-1);
   ...
   {
      string err_cmd;
      myStrGetTok(str, err_cmd);
      cerr << "Illegal command!! \"" << err_cmd <<"\"";
...
CmdParser::listCmd(const string& str)
{
   ...
   if(cmd_vec.empty())
   {
      cmd_vec.push_back("DBAPpend");
      cmd_vec.push_back("DBAVerage");
      cmd_vec.push_back("DBCount");
      cmd_vec.push_back("DBMAx");
      cmd_vec.push_back("DBMIn");
      cmd_vec.push_back("DBPrint");
      cmd_vec.push_back("DBRead");
      cmd_vec.push_back("DBSOrt");
      cmd_vec.push_back("DBSUm");
      cmd_vec.push_back("DOfile");
      cmd_vec.push_back("HELp");
      cmd_vec.push_back("HIStory");
      cmd_vec.push_back("Quit");
   }
      cout << endl;
      deleteLine();
   if(str == "")
   {
      ...
      {
         // string tmp = cmd_vec[i];
         // transform(tmp.begin(),tmp.end(),tmp.begin(),::toupper);
         cout << setw(12) << left << cmd_vec[i];
         count_line++;
         if(count_line%5==0) cout << endl;
   ...
   else if(rep.size()!=0)
   {
      ...
      for(int i = 0; i < (int)cmd_vec.size(); i++)
      {
         ...
         transform(tmp.begin(),tmp.end(),tmp.begin(),::toupper);
         string tmp_rep = rep;
         transform(tmp_rep.begin(),tmp_rep.end(),tmp_rep.begin(),::toupper);
         // if (tmp.find(tmp_rep) != string::npos)
         if(!myStrNCmp(tmp, tmp_rep, tmp_rep.size()))
         {
            cout << setw(12) << left << cmd_vec[i];
            count_line2++;
            if(count_line2%5==0) cout << endl;
         }
...
CmdParser::getCmd(string cmd)
{
   ...
   string tmp_cmd;
   int cmp_state;
   myStrGetTok(cmd, tmp_cmd);
   // cout << "After token is: " << tmp_cmd << endl;
   for(iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++)
   {
      string cur_map_key = iter->first; 
      // cout << "map key is: " << cur_map_key << endl;
      if(cur_map_key == "DO")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DOfile", tmp_cmd,2);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "HEL")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("HELp", tmp_cmd,3);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "HIS")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("HIStory", tmp_cmd,3);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "Q")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("Quit", tmp_cmd,1);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBAP")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBAPpend", tmp_cmd,4);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBAV")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBAVerage", tmp_cmd,4);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBC")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBCount", tmp_cmd,3);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBMA")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBMAx", tmp_cmd,4);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBMI")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBMIn", tmp_cmd,4);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBP")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBPrint", tmp_cmd,3);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBR")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBRead", tmp_cmd,3);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBSO")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBSOrt", tmp_cmd,4);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      else if(cur_map_key == "DBSU")
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         cmp_state = myStrNCmp("DBSUm", tmp_cmd,4);
         if(cmp_state == 0) 
         {
            e = iter->second;
            return e;
      ...
      {
         // cout <<"map key entered: " << cur_map_key << endl;
         // cout << "i am not key QQ\n";
         return 0;
      }
   }

   // cerr << "Illegal command!! \"" << tmp_cmd <<"\"";

   // cout << "the cmp_state is: " << cmp_state << endl;
