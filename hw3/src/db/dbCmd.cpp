...
DBAppendCmd::exec(const string& option)
{
  ...
  if(dbjson)
  {
    ...
    size_t first_end = myStrGetTok(option, first_option);
    myStrGetTok(option, second_option, first_end);
    // cout << "the option is: \"" << option << "\""<<endl;
    // cout << "the first option is: \"" << first_option << "\""<<endl;
    // cout << "the second option is: \"" << second_option << "\""<<endl;
    ...
    if(!isValidVarName(first_option))
    {
      cerr << "Error: Illegal option!! (" << first_option << ")\n";
      return CMD_EXEC_DONE;

    }
    int num = 0;
    if(!myStr2Int(second_option, num))
    {
      cerr << "Error: Illegal option!! (" << second_option << ")\n";
      return CMD_EXEC_DONE;

    }
    for(int i = 0; i < (int)dbjson.size(); i++)
    {
      if(dbjson[i].key() == first_option)
      {
        cerr << "Error: Element with key \"" << first_option <<"\" already exists!!\n";
        return CMD_EXEC_DONE;

      }
    }
    DBJsonElem obj(first_option, num);
    dbjson.add(obj);


  }
  else cerr << "Error: DB is not created yet!!\n";
